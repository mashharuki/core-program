...
class User {
    ...
    async proveData(data: { [key: number]: string | number }) {
        ...
        // Workaround for TypeScript type definition issue
        // The IncrementalMerkleTree does have createProof method, but TypeScript doesn't recognize it
        const stateTreeProof = (stateTree as any).createProof(index)
